...
class Embedding_sg3(nn.Module):
    ...
    def __init__(self, opts):
        ...
        self.opts = opts
        self.net,self.opts_sg3 = load_encoder(checkpoint_path=opts.stylegan3_weights)
    ...
    def invert_images_in_W3(self, image_path=None):
        ...
        for ref_im_H, ref_im_L, ref_name in ibar:
            ...
            optimizer_W, latent = self.setup_W3_optimizer()
            ...
            for step in pbar:
                ...
                if step==0:
                    avg_image_for_batch = self.avg_image.unsqueeze(0).repeat(ref_im_L.shape[0], 1, 1, 1)
                    x_input = torch.cat([ref_im_L, avg_image_for_batch], dim=1)
                    gen_im, latent = self.net.forward(x_input, 
                                                 latent=None, 
                                                 return_latents=True)
                else:
                    y_hat_clone = gen_im.clone().detach().requires_grad_(True)
                    latent_clone = latent.clone().detach().requires_grad_(True)
                    x_input = torch.cat([ref_im_L, y_hat_clone], dim=1)
                    gen_im, latent = self.net.forward(inputs=x_input, 
                                                 latent=[latent_in], 
                                                 input_code=True, 
                                                 return_latents=True,
                                                 resize=self.opts_sg3.resize_outputs)
                im_dict = {
                ...
                if self.opts.save_intermediate and step % self.opts.save_interval== 0:
                    self.save_W3_intermediate_results(ref_name, gen_im, latent_in, step)

            self.save_W3_results(ref_name, gen_im, latent_in)

    def invert_images_in_W3_with_pre_align(self, images):
        device = self.opts.device
        ibar = tqdm(self.dataloader, desc='Images')
        for ref_im_H, ref_im_L, ref_name in ibar:
            optimizer_W, latent = self.setup_W3_optimizer()
            ...
            for step in pbar:
                ...
                gen_im, _ = self.net([latent_in], input_code=True, return_latents=False)
                im_dict = {
                    'ref_im_H': ref_im_H.to(device),
                    'ref_im_L': ref_im_L.to(device),
                    'gen_im_H': gen_im,
    ...
    def invert_images_in_FS(self, image_path=None, trg_name = ''):
        ...
        for ref_im_H, ref_im_L, ref_name in ibar:
            ...
            F_init, _ = self.net([latent_W], input_code=True, return_latents=False) #start_layer=0, end_layer=3
    ...
    def save_W3_results(self, ref_name, gen_im, latent_in):
    ...
    def save_W3_intermediate_results(self, ref_name, gen_im, latent_in, step):
